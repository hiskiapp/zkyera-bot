...
function bind(zkyera) {
    ...
    zkyera.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    zkyera.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    zkyera.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    zkyera.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   zkyera.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    zkyera.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    zkyera.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    zkyera.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    zkyera.ev.on('chats.update', updates => {
    ...
    zkyera.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    zkyera.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**zkyera.ev.on('messages.update', updates => {
    ...
    zkyera.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    zkyera.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    zkyera.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (zkyera === null || zkyera === void 0 ? void 0 : zkyera.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (zkyera === null || zkyera === void 0 ? void 0 : zkyera.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (zkyera === null || zkyera === void 0 ? void 0 : zkyera.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return zkyera === null || zkyera === void 0 ? void 0 : zkyera.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (zkyera === null || zkyera === void 0 ? void 0 : zkyera.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (zkyera === null || zkyera === void 0 ? void 0 : zkyera.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (zkyera === null || zkyera === void 0 ? void 0 : zkyera.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (zkyera === null || zkyera === void 0 ? void 0 : zkyera.messageInfo(remoteJid, id))
                if (msg) {
