...
const connect = async () => {
    ...
    const zkyera = new WAConnection(global.opts["legacy"] ? makeWALegacySocket(connOptions) : makeWASocket(connOptions))
    global.Store = Store.bind(zkyera)

    zkyera.ev.on("creds.update", saveState)

    zkyera.ev.on("connection.update", async(update) => {
        if (update.connection == "open" && zkyera.type == "legacy") {
            zkyera.user = {
                id: zkyera.state.legacy.user.id,
                jid: zkyera.state.legacy.user.id,
                name: zkyera.state.legacy.user.name
            }
        }
        ...
        if (connection == "close") {
            ...
            if (reason === DisconnectReason.badSession) { console.log(`Bad Session File, Please Delete Session and Scan Again`); zkyera.logout(); }
            ...
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); zkyera.logout(); }
            ...
            else zkyera.end(`Unknown DisconnectReason: ${reason}|${connection}`)
        }
    })

    zkyera.ev.on("messages.upsert", async (chatUpdate) => {
        m = serialize(zkyera, chatUpdate.messages[0])

        if (!m.message) return
        if (m.key && m.key.remoteJid == "status@broadcast") return
        if (m.key.id.startsWith("BAE5") && m.key.id.length == 16) return
        require("./zkyera")(zkyera, m, Commands, chatUpdate)
    })

    if (zkyera.user && zkyera.user?.id) zkyera.user.jid = jidNormalizedUser(zkyera.user?.id)
    zkyera.logger = (zkyera.type == "legacy") ? DEFAULT_LEGACY_CONNECTION_CONFIG.logger.child({ }) : DEFAULT_CONNECTION_CONFIG.logger.child({ })
}
