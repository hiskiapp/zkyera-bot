...
module.exports = async (zkyera, m, commands, chatUpdate) => {
    try {
        ...
        let metadata = isGroup ? await zkyera.groupMetadata(from) : {}
        ...
        let isBotAdmin = isGroup ? groupAdmin.includes(zkyera.user?.jid) : false
        let isAdmin = isGroup ? groupAdmin.includes(sender) : false
        let isOwner = [zkyera.user?.jid, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(sender)

        if (options.autoRead) (zkyera.type == "legacy") ? await zkyera.chatRead(m.key, 1) : await zkyera.sendReadReceipt(from, sender, [m.id])
        ...
        if (isMedia && m.msg.fileSha256 && (m.msg.fileSha256.toString("hex") in global.db.sticker)) {
            ...
            let messages = await generateWAMessage(m.from, { text, mentions }, {
                userJid: zkyera.user.jid,
                quoted: m.quoted && m.quoted.fakeObj
            })
            messages.key.fromMe = await areJidsSameUser(m.sender, zkyera.user.jid)
            ...
            zkyera.ev.emit("messages.upsert", msg)
        }
        ...
        if (m.message && m.message.protocolMessage && m.message.protocolMessage.type == 0) {
            ...
            let msg = await zkyera.serializeM(await Store.loadMessage(key.remoteJid, key.id))
            let teks = `
...
⬡ Type : ${msg.type}
            `
            zkyera.sendText(m.from, teks, msg, { mentions: [msg.sender] })
            zkyera.relayMessage(m.from, msg.message, { messageId: msg.id })
        }
...
_Send command again if needed_
        ...
        try {
            ...
            zkyera.sendMessage(from, { text: String(e) }, { quoted: m })
        }   
    } catch (e) {
        console.log(e)
